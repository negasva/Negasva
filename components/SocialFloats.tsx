...
// WhatsApp SOLO para visitantes de LATAM: /api/geo decide por la cabecera
// x-vercel-ip-country (lista configurable por env LATAM_COUNTRIES). El número
// es público por definición (va en el href): NEXT_PUBLIC_WHATSAPP_NUMBER.
...
export default function SocialFloats() {
  ...
  useEffect(() => {
    ...
    fetch('/api/geo')
      .then((r) => r.json())
      .then((d) => setLatam(Boolean(d.latam)))
      .catch(() => null);
  }, []);
    ...
    >
      {latam && WHATSAPP_NUMBER && (
        <a
          ...
          className="flex items-center gap-2 rounded-full px-4 py-3 shadow-xl hover:scale-105 active:scale-95 transition-transform sm:w-44 bg-[#25D366] text-white"
        >
