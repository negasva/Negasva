# Auditoría · Integridad de precios end-to-end

**Sección:** Pagos y dinero · **Prioridad:** P0 · Crítico (hazlo esta semana)

Copia TODO lo que está debajo de la línea y pégalo en Claude Code, parado en la raíz del repo:

---

Haz una auditoría de **integridad de precios end-to-end** en este repo (negasva.shop: e-commerce
Next.js App Router + Supabase + pagos con PayPal y Mercado Pago + Printful para
productos físicos, desplegado en Vercel).

Por qué importa en este proyecto: El proyecto ya usa pricing autoritativo del servidor (buena señal), pero basta un solo campo confiado al cliente (rate de moneda, rateId de envío, add-on) para que alguien pague $1 por un pedido de $100.

Zonas clave donde empezar (no te limites a ellas — explora lo relacionado):
- app/api/pricing/quote
- app/api/checkout/route.ts
- lib/payments/paypal.ts
- app/api/payments/mercadopago/route.ts
- app/order/useCheckout.ts (params que envía el cliente)

Revisa específicamente cada uno de estos puntos y márcalos como ✅ CUMPLE /
⚠️ PARCIAL / ❌ FALLA con evidencia (archivo:línea):
- Que el servidor recalcule TODO el desglose en /api/checkout sin confiar en ningún monto del cliente (revisar cada campo de checkoutParams, incluido rate y currency).
- Que el envío se re-cotice en el servidor con Printful a partir del rateId + dirección (nunca aceptar el precio de envío del cliente).
- Que el descuento por código se recalcule y se limite en el servidor (cap correcto, no negativo, no acumulable indebidamente).
- Que la conversión de moneda use una tasa del servidor, no el 'rate' que manda el navegador.
- Que la orden PayPal/MP se cree con el total del servidor y que reference/custom_id no sea manipulable.
- Probar activamente: modificar payloads con montos alterados y confirmar que el cobro no cambia.

ENTREGABLES (no apliques fixes de código en esta pasada — solo audita y documenta):

1. `audits/pagos-y-dinero/integridad-precios/REPORTE.md` — hallazgos ordenados por severidad
   (crítico / alto / medio / bajo). Cada hallazgo con: evidencia (archivo:línea),
   explicación del riesgo en una frase, fix propuesto concreto, y esfuerzo estimado
   (S/M/L). Incluye al final una tabla-resumen con columna "Estado" para ir marcando.

2. `audits/pagos-y-dinero/integridad-precios/BUENAS_PRACTICAS.md` — reglas accionables y
   permanentes de integridad de precios end-to-end para ESTE proyecto, escritas en imperativo
   (ej. "Nunca X", "Siempre Y al tocar Z"), derivadas tanto de lo que ya se hace
   bien como de los fallos encontrados. Este archivo es la referencia que cualquier
   sesión futura de Claude Code debe seguir al tocar código de esta área.

3. Agrega (si no existe) una línea en `CLAUDE.md` bajo una sección
   "## Buenas prácticas por área" que diga: antes de tocar código relacionado con
   pagos y dinero, leer `audits/pagos-y-dinero/integridad-precios/BUENAS_PRACTICAS.md`.

4. Commit de los .md siguiendo el flujo de git definido en CLAUDE.md (commit, push,
   PR con auto-merge), sin tocar código de la app en esta tanda.

Si durante la auditoría encuentras algo CRÍTICO explotable ahora mismo (fuga de
datos, bypass de pago), márcalo al inicio del reporte con 🚨 y dímelo de inmediato
en tu respuesta, no lo dejes enterrado en el archivo.
