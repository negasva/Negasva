# Auditoría · Rate limiting y anti-abuso
...
Haz una auditoría de **rate limiting y anti-abuso** en este repo (negasva.shop: e-commerce
Next.js App Router + Supabase + pagos con PayPal y Mercado Pago + Printful para
productos físicos, desplegado en Vercel).

Por qué importa en este proyecto: Sin Redis compartido (Upstash), el rate limiting cae a memoria por instancia serverless: en Vercel eso significa que casi no limita. Bots pueden spamear checkout, upload y probar códigos de descuento por fuerza bruta.

Zonas clave donde empezar (no te limites a ellas — explora lo relacionado):
- lib/security/apiHelpers.ts
- .env (UPSTASH_REDIS_REST_URL/TOKEN)
- lib/security/recaptchaClient.ts
...
- Que UPSTASH_REDIS_REST_URL/TOKEN estén configurados en producción (si no, el rate limit es por instancia y casi inútil).
- Que TODOS los endpoints costosos tengan límite: checkout, upload, quote, capture, newsletter, login admin, validación de códigos.
- Que la verificación de reCAPTCHA en el servidor esté activa (RECAPTCHA_SECRET_KEY definido) y no en modo 'se omite'.
- Que probar códigos de descuento tenga un límite estricto por IP (es un objetivo clásico de fuerza bruta).
- Que los límites tengan valores sensatos y devuelvan 429 con mensaje claro.

ENTREGABLES (no apliques fixes de código en esta pasada — solo audita y documenta):

1. `audits/seguridad/rate-limiting-antiabuso/REPORTE.md` — hallazgos ordenados por severidad
...
2. `audits/seguridad/rate-limiting-antiabuso/BUENAS_PRACTICAS.md` — reglas accionables y
   permanentes de rate limiting y anti-abuso para ESTE proyecto, escritas en imperativo
...
3. Agrega (si no existe) una línea en `CLAUDE.md` bajo una sección
   ...
   seguridad, leer `audits/seguridad/rate-limiting-antiabuso/BUENAS_PRACTICAS.md`.
