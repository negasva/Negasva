# Auditoría · Secretos y dependencias vulnerables
...
Haz una auditoría de **secretos y dependencias vulnerables** en este repo (negasva.shop: e-commerce
Next.js App Router + Supabase + pagos con PayPal y Mercado Pago + Printful para
productos físicos, desplegado en Vercel).

Por qué importa en este proyecto: Una clave filtrada en el historial de git o un paquete npm con CVE conocido comprometen todo lo demás aunque el código propio sea perfecto.
...
- Escanear TODO el historial de git (no solo HEAD) buscando claves: service role de Supabase, PAYPAL_SECRET, tokens de MP/Printful/Resend, etc. (gitleaks o trufflehog).
- Que .env.local y variantes estén en .gitignore y nunca commiteadas.
- Que ninguna variable sensible use prefijo NEXT_PUBLIC_ por error (eso la mete al bundle del navegador).
- npm audit: identificar vulnerabilidades high/critical en dependencias y plan de actualización.
- Que las dependencias de pago (@paypal/react-paypal-js, SDK de MP) estén en versiones vigentes.

ENTREGABLES (no apliques fixes de código en esta pasada — solo audita y documenta):

1. `audits/seguridad/secretos-dependencias/REPORTE.md` — hallazgos ordenados por severidad
...
2. `audits/seguridad/secretos-dependencias/BUENAS_PRACTICAS.md` — reglas accionables y
   permanentes de secretos y dependencias vulnerables para ESTE proyecto, escritas en imperativo
...
3. Agrega (si no existe) una línea en `CLAUDE.md` bajo una sección
   ...
   seguridad, leer `audits/seguridad/secretos-dependencias/BUENAS_PRACTICAS.md`.
