# Auditoría · Analítica y eventos de negocio

**Sección:** Datos y privacidad · **Prioridad:** P2 · Mejora (cuando lo anterior esté hecho)

Copia TODO lo que está debajo de la línea y pégalo en Claude Code, parado en la raíz del repo:

---

Haz una auditoría de **analítica y eventos de negocio** en este repo (negasva.shop: e-commerce
Next.js App Router + Supabase + pagos con PayPal y Mercado Pago + Printful para
productos físicos, desplegado en Vercel).

Por qué importa en este proyecto: Sin eventos bien instrumentados no sabes dónde pierdes ventas ni si un cambio mejoró algo. Hoy hay GA y Ahrefs, pero ¿registran el funnel y las compras con montos correctos?

Zonas clave donde empezar (no te limites a ellas — explora lo relacionado):
- NEXT_PUBLIC_GA_ID (integración GA)
- eventos en app/order/**
- webhook (evento purchase server-side)

Revisa específicamente cada uno de estos puntos y márcalos como ✅ CUMPLE /
⚠️ PARCIAL / ❌ FALLA con evidencia (archivo:línea):
- Que exista el evento 'purchase' con monto, moneda e items — idealmente disparado server-side desde el webhook (el redirect del cliente se pierde a veces).
- Eventos de cada paso del wizard para medir el funnel.
- Que los montos reportados cuadren con lo cobrado (validar contra pedidos reales).
- UTM/atribución: saber de qué canal viene cada venta (Instagram/TikTok del footer).
- Que analytics no rompa la CSP (ya pasó con Ahrefs — verificar tras el fix).

ENTREGABLES (no apliques fixes de código en esta pasada — solo audita y documenta):

1. `audits/datos-y-privacidad/analitica-eventos/REPORTE.md` — hallazgos ordenados por severidad
...
2. `audits/datos-y-privacidad/analitica-eventos/BUENAS_PRACTICAS.md` — reglas accionables y
   permanentes de analítica y eventos de negocio para ESTE proyecto, escritas en imperativo
...
3. Agrega (si no existe) una línea en `CLAUDE.md` bajo una sección
   ...
   datos y privacidad, leer `audits/datos-y-privacidad/analitica-eventos/BUENAS_PRACTICAS.md`.
