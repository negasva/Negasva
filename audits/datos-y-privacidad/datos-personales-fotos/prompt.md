# Auditoría · Ciclo de vida de fotos y datos personales
...
Haz una auditoría de **ciclo de vida de fotos y datos personales** en este repo (negasva.shop: e-commerce
Next.js App Router + Supabase + pagos con PayPal y Mercado Pago + Printful para
productos físicos, desplegado en Vercel).

Por qué importa en este proyecto: Guardas fotos de familias (a veces niños) para pintar retratos. Definir cuánto tiempo se guardan, quién accede y cómo se borran no es opcional: es lo mínimo que un cliente esperaría.

Zonas clave donde empezar (no te limites a ellas — explora lo relacionado):
- app/api/order/upload/route.ts
- bucket privado de Supabase
- supabase/migrations/019_order_photos.sql
- panel admin

Revisa específicamente cada uno de estos puntos y márcalos como ✅ CUMPLE /
⚠️ PARCIAL / ❌ FALLA con evidencia (archivo:línea):
- Inventariar qué datos personales se guardan y dónde: fotos, nombre, email, WhatsApp, dirección de envío.
- Política de retención: ¿cuánto tiempo se guardan las fotos tras entregar el pedido? ¿Y las de pedidos nunca pagados?
- Acceso: ¿solo el admin puede ver fotos? ¿Las URLs firmadas expiran rápido?
- Borrado: ¿existe un proceso para eliminar datos de un cliente que lo pida?
- Fotos de pedidos abandonados (subidas pero nunca pagadas): son datos huérfanos que hay que purgar.

ENTREGABLES (no apliques fixes de código en esta pasada — solo audita y documenta):

1. `audits/datos-y-privacidad/datos-personales-fotos/REPORTE.md` — hallazgos ordenados por severidad
...
2. `audits/datos-y-privacidad/datos-personales-fotos/BUENAS_PRACTICAS.md` — reglas accionables y
   permanentes de ciclo de vida de fotos y datos personales para ESTE proyecto, escritas en imperativo
...
3. Agrega (si no existe) una línea en `CLAUDE.md` bajo una sección
   ...
   datos y privacidad, leer `audits/datos-y-privacidad/datos-personales-fotos/BUENAS_PRACTICAS.md`.
