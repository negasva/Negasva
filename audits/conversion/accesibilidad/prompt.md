...
Haz una auditoría de **accesibilidad (a11y)** en este repo (negasva.shop: e-commerce
Next.js App Router + Supabase + pagos con PayPal y Mercado Pago + Printful para
productos físicos, desplegado en Vercel).

Por qué importa en este proyecto: Contraste, teclado y lectores de pantalla amplían tu mercado, mejoran SEO y reducen riesgo legal. El rosa claro sobre blanco del branding es un punto débil típico de contraste.

Zonas clave donde empezar (no te limites a ellas — explora lo relacionado):
- tokens de color del design system
- app/order/page.tsx
- componentes interactivos del wizard

Revisa específicamente cada uno de estos puntos y márcalos como ✅ CUMPLE /
⚠️ PARCIAL / ❌ FALLA con evidencia (archivo:línea):
- Contraste WCAG AA: el texto rosa claro (#FC90B6) sobre blanco casi seguro falla — medir cada combinación de texto.
- Navegar TODO el wizard solo con teclado: focus visible, orden lógico, sin trampas.
- Labels reales en todos los inputs; errores anunciados con aria-live.
- Alt text significativo en imágenes de catálogo.
- Correr axe-core/Lighthouse a11y en las páginas principales y clasificar los hallazgos.

ENTREGABLES (no apliques fixes de código en esta pasada — solo audita y documenta):

1. `audits/conversion/accesibilidad/REPORTE.md` — hallazgos ordenados por severidad
...
2. `audits/conversion/accesibilidad/BUENAS_PRACTICAS.md` — reglas accionables y
   permanentes de accesibilidad (a11y) para ESTE proyecto, escritas en imperativo
...
3. Agrega (si no existe) una línea en `CLAUDE.md` bajo una sección
   ...
   conversión y ux de venta, leer `audits/conversion/accesibilidad/BUENAS_PRACTICAS.md`.
